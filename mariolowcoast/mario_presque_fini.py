...
bg=pygame.image.load('blabla2.png')  ##import des images 
...
def gener_mob(t,lv): #apparition des mobs de manière aléatoire sur la map, aux alentours du personnage 
    if (t*lv)%400==0:
        mob.append(mob_rat(random.randrange(int(mario.x-400), int(mario.x+400)), parachute[lv-1]))
    if (t*lv)%1000==0:
        mob.append(mob_pain(random.randrange(int(mario.x-400), int(mario.x+400)),parachute[lv-1]))
        paing.append(mob[-1])
